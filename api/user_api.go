...
func SetUpUserAPI(router *gin.Engine, dbClient *mongo.Client) {
	db.InitializeUserDatabaseSetting(dbClient)
	userGroup := router.Group("/users")
	userGroup.GET("/", getUserList)
	//userGroup.GET("/:id", getUserByID)
	userGroup.POST("/", createUser(dbClient))
	userGroup.POST("/info", getUserID)
...
func getUserID(c *gin.Context) {
	...
	email, emailExists := requestBody["email"].(string)
	password, passwordExists := requestBody["password"].(string)
